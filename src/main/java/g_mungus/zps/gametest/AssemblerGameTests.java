    ...
    public static void setRecipeFulfillableFilter(GameTestHelper helper) {
        ...
        level.getRecipeManager().getAllRecipesFor(RecipeType.CRAFTING).stream()
                .filter(recipe -> recipe.getIngredients().isEmpty()
                        || recipe.getIngredients().stream().allMatch(Ingredient::isEmpty))
                .findFirst()
                .ifPresent(special -> {
                    if (AssemblerRecipeSupport.isFulfillable(special) || ids.contains(special.getId())) {
                        helper.fail("special recipe " + special.getId() + " should not be fulfillable");
                    }
                });
        helper.succeed();
    }
