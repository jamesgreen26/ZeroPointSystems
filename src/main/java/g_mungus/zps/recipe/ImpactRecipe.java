...
public class ImpactRecipe implements Recipe<ImpactInput> {
    ...
    public ImpactRecipe(ResourceLocation id, HolderSet<Block> ingredient, List<ImpactResult> results) {
        this(id, ingredient, Optional.empty(), results);
    }

    public ImpactRecipe(ResourceLocation id, HolderSet<Block> ingredient, Optional<StatePropertiesPredicate> properties, List<ImpactResult> results) {
        this.id = id;
    ...
    @Override
    public @NotNull ItemStack assemble(@NotNull ImpactInput input, @NotNull RegistryAccess registries) {
    ...
    public @NotNull ItemStack getResultItem(@NotNull RegistryAccess registries) {
        ImpactResult best = results.get(0);
        for (ImpactResult result : results) {
