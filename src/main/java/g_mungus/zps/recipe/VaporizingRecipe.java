...
public class VaporizingRecipe implements Recipe<VaporizingInput> {
    ...
    public VaporizingRecipe(ResourceLocation id, List<Ingredient> ingredients, List<GasOutput> results,
        ...
        for (int slot = 0; slot < input.size(); slot++) {
            ...
            for (int other = 0; other < input.size(); other++) {
                ...
                if (other != slot && remaining[other] < otherStack.getCount()
                        && ItemStack.isSameItemSameTags(stack, otherStack)) {
                    takenElsewhere = true;
                    break;
                }
            }
    ...
    @Override
    public @NotNull ItemStack assemble(@NotNull VaporizingInput input, @NotNull RegistryAccess registries) {
