...
public final class AssemblerRecipeSupport {
    ...
    public static Set<ResourceLocation> fulfillableIds(Level level) {
        ...
        for (RecipeHolder<Shaped5x5Recipe> holder : recipeManager.getAllRecipesFor(ModRecipes.SHAPED_5X5_TYPE.get())) {
            if (isFulfillable(holder.value())) {
                ids.add(holder.id());
            }
        }
        ...
        if (Compat.isCreateLoaded()) {
            for (RecipeHolder<CraftingRecipe> holder : MechanicalCraftingCompat.mechanicalCraftingRecipes(level)) {
                if (isFulfillable(holder.value())) {
                    ids.add(holder.id());
                }
            }
