...
public class LidarScannerBlockEntity extends NetworkTerminalImpl implements LightPipeDataSender {
    ...
    public void tick() {
        ...
        if ((serverLevel.getGameTime() + tickOffset) % TICK_FREQUENCY != 0) {
            return;
        }
        ...
        ScanResult scanResult = runScan(serverLevel, state.getValue(AbstractRadioBlock.FACING));
    ...
    private ScanResult runScan(ServerLevel level, Direction facing) {
        ...
        for (int row = 0; row < GRID_SIZE; row++) {
            ...
            for (int col = 0; col < GRID_SIZE; col++) {
                ...
                double distance = LidarRaycasts.raycast(level, start, direction, MAX_CAST_DISTANCE, scanContext);
                if (distance >= 0.0) {
                    hit = true;
                }
