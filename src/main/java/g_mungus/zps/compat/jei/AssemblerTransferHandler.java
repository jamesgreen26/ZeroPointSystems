 ...
 */
public class AssemblerTransferHandler<R extends Recipe<?>> implements IRecipeTransferHandler<AssemblerMenu, R> {
    private final IRecipeTransferHandlerHelper helper;
    private final RecipeType<R> recipeType;

    public AssemblerTransferHandler(IRecipeTransferHandlerHelper helper, RecipeType<R> recipeType) {
        this.helper = helper;
    ...
    @Override
    public @Nullable IRecipeTransferError transferRecipe(AssemblerMenu container, R recipe,
                                                        IRecipeSlotsView recipeSlots, Player player,
                                                        boolean maxTransfer, boolean doTransfer) {
        // Rules out dynamic recipes (no fixed ingredients) and anything too big for the 5x5 grid.
        if (!AssemblerRecipeSupport.isFulfillable(recipe)) {
