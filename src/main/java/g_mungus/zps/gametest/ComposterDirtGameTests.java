...
public class ComposterDirtGameTests {
    ...
    public static void itemHandler_offersDirtFromBelowOnly(GameTestHelper helper) {
        ...
        IItemHandler side = sidedHandler(helper, absolute, Direction.NORTH);
        if (side != null && side.getSlots() != 0) {
            helper.fail("The side faces should expose no slots", COMPOSTER_POS);
        }
        IItemHandler below = sidedHandler(helper, absolute, Direction.DOWN);
    ...
    /** What a hopper or pipe sees on that face: Forge wraps a {@link WorldlyContainerHolder} block for it. */
    private static IItemHandler sidedHandler(GameTestHelper helper, BlockPos absolute, Direction side) {
        BlockState state = helper.getLevel().getBlockState(absolute);
        if (!(state.getBlock() instanceof WorldlyContainerHolder holder)) {
            return null;
        }
        return new SidedInvWrapper(holder.getContainer(state, helper.getLevel(), absolute), side);
    }
}
