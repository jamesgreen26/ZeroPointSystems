...
- Ores are covered automatically: one recipe keyed on `#c:ores_in_ground/stone` pounds any stone-hosted ore, modded ones included, into suspicious gravel holding what the ore would have dropped to a Fortune II pickaxe. It works through a result's `buried_drops` field (`{ "fortune": 2 }`), which rolls the struck block's own loot table at the moment of impact instead of burying a named item.
- JEI lists what a suspicious result may be hiding as item icons under "May contain:", read from the result's `buried_item`. For the ore recipe that is the `zps:stone_ores_in_ground_drops` item tag, which nobody maintains by hand: at the end of each data load the server fills it with the items named in the loot table of each block in `#c:ores_in_ground/stone` (raw iron, diamond, and so on, minus the silk touch branch that returns the ore itself). Datapacks can still add to it.
